...
export const login = async (
    ...
    try {
        ...
        if (!accessTokenSecret)
            return res
                .status(400)
                .json({ success: false, message: "missing JWT_ACCESS_TOKEN_SECRET in .env" });
        ...
        return res
            ...
            .json({ success: true, message: "Login successfully.", access_token: accessToken });
    } catch (err) {
        return res
            .status(400)
            .json({ success: false, message: "Login failed, please verify your credentials." });
    }
};
