...
export const login = async (req: Request, res: Response, next: NextFunction) => {
    ...
    const query = `SELECT id_usuario, username, password, nombre, apellido, email FROM usuario WHERE username = ? AND is_active = 1`
...
export const getUserData = async (req: Request, res: Response, next: NextFunction) => {
    ...
    // 2. Validate username and password exists
    if (!access_token) return res.status(500).json({ message: 'access_token is required' })
    // 3. Extract data from access_token
    const accessTokenSecret = process.env.JWT_ACCESS_TOKEN_SECRET
    if (!accessTokenSecret) return res.status(500).json({ message: "missing JWT_ACCESS_TOKEN_SECRET in .env" })
    try {
        jwt.verify(access_token, accessTokenSecret)
        
    // 3. Search for user in db
    const user_id = 1
    const query = `SELECT username, nombre, apellido, email FROM usuario WHERE id_usuario = ?`
    pool.execute(query, [user_id], async (err, result) => {
        if (err) throw err
        const users = result as UserRow[]
        const userFound = users[0]
        if (!userFound) return res.status(401).json({ message: "User or password is wrong, try again." })
        ...
        // 6. Return 200 with access token
        return res.status(200).json({ message: `User with ID: "${userDataJSON.id}"  data found.`, results: userDataJSON })
    })
    } catch(err) {
        // TODO: GENERATE CUSTOM RESPONSE FOR BAD TOKEN
